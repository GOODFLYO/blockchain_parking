<template>
    <div class="AddPositionTude toolsView">
      <button @click="addPositionTude" class="css-button-arrow--red">
        添加经纬度
      </button>
      <input type="text" v-model="lg" placeholder="请输入经度" />
      <input type="text" v-model="lt" placeholder="请输入纬度" />
    </div>
    <p>{{ info }}</p>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      async addPositionTude() {
        await this.linkWallet();
        const { bpContractWithSigner } = await this.createContractWithSigner();
  
        try {
          const tx = await bpContractWithSigner.addPositionTude(this.lg,this.lt);
          await tx.wait();
          console.log("添加成功");
          this.info = "添加成功";
        } catch (error) {
