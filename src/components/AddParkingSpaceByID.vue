<template>
    <div class="AddParkingSpaceByID toolsView">
      <button @click="addParkingSpaceByID" class="css-button-arrow--red">
        上线固定车位
      </button>
      <input type="text" v-model="id" placeholder="请输入上线车位号" />
    </div>
    <p>{{ info }}</p>
  </template>
  ...
  export default {
    ...
    methods: {
      ...
      async addParkingSpaceByID() {
        await this.linkWallet();
        const { bpContractWithSigner } = await this.createContractWithSigner();
  
        try {
          const tx = await bpContractWithSigner.addParkingSpace(this.id);
          await tx.wait();
          console.log("添加成功");
          this.info = "添加成功";
        } catch (error) {
