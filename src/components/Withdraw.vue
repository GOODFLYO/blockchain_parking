<template>
  <div class="Withdraw toolsView">
    <button @click="withdraw" class="css-button-arrow--red">用户提现</button>
    <input type="text" v-model="wd" placeholder="请输入提现数量" />
  </div>
  <p>{{ info }}</p>
</template>
...
export default {
  ...
  methods: {
    ...
    async withdraw() {
      ...
      try {
        const tx = await bpContractWithSigner.withdraw(this.wd);
        await tx.wait();
        console.log("添加成功");
