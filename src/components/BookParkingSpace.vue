<template>
  <div class="BookParkingSpace toolsView">
    <button @click="bookParkingSpace" class="css-button-arrow--red">
      预定车位
    </button>
    <input type="text" v-model="id" placeholder="请输入预约车位ID" />
    <input
      type="text"
      v-model="starttime"
      placeholder="请输入开始时间(时间戳)"
    />
    <input type="text" v-model="duration" placeholder="请输入停车时长(秒)" />
  </div>
  <p>{{ info }}</p>
</template>
...
export default {
  ...
  methods: {
    ...
    async bookParkingSpace() {
      ...
      try {
        const tx = await bpContractWithSigner.bookParkingSpace(
          this.id,
          this.starttime,
          this.duration
        );
