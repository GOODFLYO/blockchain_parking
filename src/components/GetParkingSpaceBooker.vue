<template>
    <div class="GetParkingSpaceBooker toolsView">
      <button @click="getParkingSpaceBooker" class="css-button-arrow--red">
        查看车位编号预约者
      </button>
  ...
  export default {
    ...
    methods: {
      ...
      async getParkingSpaceBooker() {
        await this.linkWallet();
        const { bpContractWithSigner } = await this.createContractWithSigner();
  
        try {
          const tx = await bpContractWithSigner.getParkingSpaceBooker(this.id);
          await tx.wait();
          console.log("添加成功");
          this.info = "添加成功";
        } catch (error) {
